...
function write_state() {
  ...
  echo "BRD$INDEX   \"--brd$INDEX \"  c (0,10,160,170,20,50,100,120) | as.numeric(S$INDEX) %in% c(0,1,2,3,4,5)" >> ${TXT_FILE}
  echo "MES$INDEX   \"--mes$INDEX \"  c (10,160,170,20,50,100,120) | as.numeric(S$INDEX) %in% c(6,7)" >> ${TXT_FILE}
  if [ ${INDEX} == 0 ]; then
...
function write_connection() {
  ...
  echo "M${STATE}x$CONNECTION  \"--m${STATE}x$CONNECTION \" c   (0,10,160,170,20,50,100,120) | as.numeric(C${STATE}x$CONNECTION) %in% c(0,1,2,3,4,5) " >> ${TXT_FILE}
