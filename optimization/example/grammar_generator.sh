...
function write_state() {
  ...
  echo "MES$INDEX   \"--mes$INDEX \"  c (10,160) | as.numeric(S$INDEX) %in% c(6,7)" >> ${TXT_FILE}
  if [ ${INDEX} == 0 ]; then
...
function write_connection() {
  ...
  echo "M${STATE}x$CONNECTION  \"--m${STATE}x$CONNECTION \" c   (0,85) | as.numeric(C${STATE}x$CONNECTION) %in% c(0,1,2,3,4,5) " >> ${TXT_FILE}
