...
function write_state() {
  ...
  echo "BRDA$INDEX   \"--brd$INDEX \"  c (0,10,160,170) | as.numeric(S$INDEX) %in% c(0,1,2,3,4,5)" >> ${TXT_FILE}
...
function write_connection() {
  ...
  echo "M${STATE}x$CONNECTION  \"--m${STATE}x$CONNECTION \" c   (10,160,170) | as.numeric(C${STATE}x$CONNECTION) %in% c(6,7) " >> ${TXT_FILE}
}
