...
function write_state() {
  ...
  echo "BRD$INDEX   \"--brd$INDEX \"  c (0,10,160,170) | as.numeric(S$INDEX) %in% c(0,1,2,3,4,5)" >> ${TXT_FILE}
  echo "MES$INDEX   \"--mes$INDEX \"  c (10,160,170) | as.numeric(S$INDEX) %in% c(6,7)" >> ${TXT_FILE}
  if [ ${INDEX} == 0 ]; then
    echo "NumConnections$INDEX \"--n$INDEX \" i (1,6) | as.numeric(NumStates)>1" >> ${TXT_FILE}
  else
    echo "NumConnections$INDEX \"--n$INDEX \" i (1,6) | as.numeric(NumStates)>${INDEX}" >> ${TXT_FILE}
  fi
}
...
function write_connection() {
  ...
  CONNECTION=$2
  create_range STATE
  echo "N${STATE}x$CONNECTION  \"--n${STATE}x$CONNECTION \" i   (0,5) | as.numeric(NumConnections$STATE)>$CONNECTION " >> ${TXT_FILE}
  ...
  echo "M${STATE}x$CONNECTION  \"--m${STATE}x$CONNECTION \" c   (0,10,160,170) | as.numeric(C${STATE}x$CONNECTION) %in% c(0,1,2,3,4,5) " >> ${TXT_FILE}
